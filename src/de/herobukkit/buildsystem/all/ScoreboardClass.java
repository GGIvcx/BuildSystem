...
public class ScoreboardClass {
	...
	public static void sendScoreboard(Player player) {
		...
		if(player.hasPermission("build.admin")) {
			objective.getScore("§8 § " + ChatColor.translateAlternateColorCodes ('&', Main.getInstance().getConfig().getString("Buildsystem.Prefix.Admin"))).setScore(14);
		} else if(player.hasPermission("build.team")) {
			objective.getScore("§8 § " + ChatColor.translateAlternateColorCodes ('&', Main.getInstance().getConfig().getString("Buildsystem.Prefix.Builder"))).setScore(14);
		} else {
			objective.getScore("§8 § " + ChatColor.translateAlternateColorCodes ('&', Main.getInstance().getConfig().getString("Buildsystem.Prefix.Visitor"))).setScore(14);
		}
