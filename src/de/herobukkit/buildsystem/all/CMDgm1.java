			...
			if (sender instanceof Player) {
				...
				if(p.hasPermission("build.team")){
					if(args.length == 0) {
						...
						//}
						p.sendMessage(ChatColor.translateAlternateColorCodes ('&', Main.getInstance().getConfig().getString("Buildsystem.Prefix.System")) + " §8| §7Now you can §eBuild");
						p.setGameMode(GameMode.CREATIVE);
						} else {
							p.sendMessage(ChatColor.translateAlternateColorCodes ('&', Main.getInstance().getConfig().getString("Buildsystem.Prefix.System")) + " §8| §7Use §e/build");
						}
